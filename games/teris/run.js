...
const SHAPES_MAP = ['L', 'O', 'I', 'Z', 'T'];
const SHAPE_INIT_LOCATION = [5, 0];
export default class Teris {
    constructor(oCanvas) {
        this.data = [[0]];
        this.shape_location = SHAPE_INIT_LOCATION;
        this.shape = null;
        this.shape_index = 0; // 当前形状的第几形态
        this.speed = 1000;
    ...
    draw_by_points(points) {
        let ctx = this.context;
        for (let i = 0; i < points.length; i++) {
            this.draw_point(points[i]);
        }
    }
    draw_by_relative_points(points, origin) {
        let ctx = this.context;
        for (let i = 0; i < points.length; i++) {
    ...
    draw_relative_point(point, origin) {
